...
export default function Page() {
  ...
  const [filteredExamplePrompt, setFilteredExamplePrompt] = useState(
    examplePrompt.generateAnimationPrompt
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (userPrompt.length < 50) {
      const rewritePrompt = await promptDesoger({
        user_prompt: userPrompt,
        instruction_type: activeTab,
      });
  ...
  return (
    <div className="flex flex-col w-full gap-2 bg-gray-200 dark:bg-slate-600">
      <Headers />
      {error && (
        <div className="bg-red-500 text-white p-2 rounded">エラー: {error}</div>
      )}
      <div className="flex w-full gap-2 justify-center">
        ...
        <div className="flex flex-col w-[50%] ">
          <div className="my-10 ml-20">
            <VideoComponent videoUrl={videoUrl} loading={loading} />
          </div>
          <div className="ml-20">
