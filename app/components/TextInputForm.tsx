...
const TextInputForm = ({
  ...
  const Button = (loading: boolean, input: string) => {
    if (loading) {
      ...
      return (
        <button
          type="submit"
          className={`absolute right-0 bottom-4 mb-1 mr-4 w-10 h-10 rounded-full flex items-center justify-center ${
            input
              ? "bg-sky-400 dark:bg-slate-600"
  ...
  return (
    <div className="flex flex-col w-full gap-2">
      ...
      >
        <textarea
          className="w-full h-40 px-5 py-3 pb-10 outline-none resize-none rounded-3xl shadow-lg text-[#334155] dark:bg-slate-700 dark:text-white"
          placeholder="文章を入力して下さい"
          value={input}
          onChange={(e) => setInput(e.target.value)}
        />
        <div>{Button(loading, input)}</div>
      </form>
    </div>
  );
};
