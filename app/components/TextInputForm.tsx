...
const TextInputForm = ({ input, handleSubmit, setInput, loading }: formProps) => {
  const { text } = useText(); // コンテキストからテキストを取得

  useEffect(() => {
    if (text) {
      setInput(text); // コンテキストの値をセット
    }
  }, [text, setInput]);
  ...
  const Button = (loading: boolean, input: string) => {
    if (loading) {
      return (
        <button type="submit" className="absolute right-0 bottom-4 mb-1 mr-4 w-10 h-10 rounded-full flex items-center justify-center bg-gray-400 cursor-not-allowed" disabled>
          <LoadingDots />
        </button>
      );
    } else {
